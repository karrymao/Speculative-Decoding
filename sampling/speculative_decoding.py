...
from utils.logits_processor import LogitsProcessor, GreedyProcessor, RandomProcessor
...
random_processor=RandomProcessor(temperature=1)
...
@torch.no_grad()
def speculative_generate_multi(
    inputs: List[int],
    drafter: Module,
    target: Module,
    tokenizer = None,
    gamma: int = 5,
    trial: int = 5,
    prune_width: int = 0,  # 0 means no pruning
    logits_processor: LogitsProcessor = GreedyProcessor(),
    max_gen_len: int = 40,
    eos_tokens_id: int | List[int] = 1,
    pad_token_id: int = 0,
    use_cache: bool = False,
    skip_sample_adjustment: bool = True,  # TODO: implement this
    first_target: bool = True,
    debug: bool = False,
) -> Tuple[List[int], float]:
    """
    ...
    Args:
        ...
        tokenizer: tokenizer (used for debugging).
        gamma (int): number of drafts generated by the drafter at each step.
        trial (int): number of trials to run the drafter.
        logits_processor (LogitsProcessor): logits processor for sampling.
        max_gen_len (int): maximum length of the generated sequence.
        eos_tokens_id (int or List[int]): end token id (could be multiple).
        pad_token_id (int): pad token id.
    ...
    while current_position < total_len:
        ...
        attention_mask[0, -len(tree.nodelist):, -len(tree.nodelist):] = tree.attention_mask()

        # run target model on drafts and get logits of the previous tokens plus one more token
        Mp = target(
            input_ids=input_ids[..., :sequence_len],
            attention_mask=attention_mask,  # NOTE: Core logic here!
            past_key_values=target_cache,  # TODO: maintain the cache
            use_cache=use_cache,
        )
        target_cache = Mp.past_key_values
        draft_logits = Mp.logits[..., current_position - 1:sequence_len, :]  # [1, len(tree.nodelist), vocab_size]
        p = logits_processor(draft_logits) # [1, len(tree.nodelist), vocab_size]
        
        # compute the last accepted draft position (rejection sampling)
        token_ids, p = tree.longest_accepted_sequence(p)
        n = len(token_ids) - 1 # exclude the root node
        drafts_accepted += n
        input_ids[0, current_position:current_position + speculated_tokens] = pad_token_id
        input_ids[0, current_position:current_position + n] = torch.tensor(token_ids[1:], dtype=torch.long, device=target.device)
        ...
        if n == corrected_gamma:
            ...
            if not skip_sample_adjustment: # TODO: implement this (q is not maintained for now)
                p_p = max_fn(p[..., n, :] - q[0, n, :])
            else:
                p_p = p[..., n, :]
        x = logits_processor.sample(p_p)
